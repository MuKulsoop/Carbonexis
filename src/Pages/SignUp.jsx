...
export default function SignUpPage() {
  ...
  const handleInputChange = (e) => {
    const { name, value } = e.target; // Change `username` to `name`
              ...
              <FadeIn direction="right" delay={0.3} fullWidth>
                <div className="flex flex-col items-center justify-center bg-inherit relative min-h-[500px] lg:min-h-screen w-full p-8">
                  <div className="h-auto relative text-center z-[5] max-w-md mx-auto">
                    <h1 className="text-[6rem] font-bold text-white -mt-40">
                      Join Us!
                    </h1>
                    ...
                    <div className="mt-8 flex flex-col gap-4">
                      <Button
                        variant="outline"
                        className={`w-full text-xl border-[#661953] text-[#661953] py-7 rounded-3xl transition-transform transform ${
                          selection === "User"
                            ? "scale-105 border-[#661953]"
                            : "hover:scale-105 hover:text-[#661953eb]"
                        }`}
              ...
              >
                <form onSubmit={handleSubmit}>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="w-full py-4 rounded-full bg-gradient-to-r from-[#1F3278] to-[#162053] text-white text-xl font-semibold hover:opacity-90 transition-opacity mt-6"
                  >
                    Sign Up
                  </motion.button>
                </form>
                ...
                <div className="text-center z-50">
                  <p className="text-white text-lg mb-2">
                    ...
                    <Link
                      to="/login"
                      className="text-[#8BA4FF] hover:text-[#9FB5FF] transition-colors cursor-pointer z-100"
                    >
                      Sign In
                    </Link>
                  </p>
                ...
                >
                  {[{ name: "Google", bg: "bg-white" }, { name: "facebook", bg: "bg-[#1877F2]" }, { name: "apple", bg: "bg-white" }].map(
                    (provider) => (
                      <motion.button
                        key={provider.name}
